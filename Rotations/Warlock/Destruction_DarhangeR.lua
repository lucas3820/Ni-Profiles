...
-----------------------------------
	["Universal pause"] = function()
		if IsMounted()
		 ...
		 and not ni.vars.followEnabled == 1) then
			return true
		end
	end,
-----------------------------------
...
-----------------------------------
	["Fel Domination"] = function()
		if not UnitExists("playerpet")
		 ...
		 and ni.spell.available(ni.data.darhanger.warlock.petDest)
		 and IsUsableSpell(GetSpellInfo(ni.data.darhanger.warlock.petDest))
	...
	["Summon pet (Imp)"] = function()
		if not UnitExists("playerpet")
		 ...
		 and GetTime() - ni.data.darhanger.warlock.LastSummon > 2 then
			ni.spell.cast(ni.data.darhanger.warlock.petDest)
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
		 ni.debug.popup("Destruction Warlock by DarhangeR", 
...
ni.bootstrap.rotation("Destruction_DarhangeR", queue, abilities, data)